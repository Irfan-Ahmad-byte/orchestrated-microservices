...
echo "PostgreSQL is up - starting users-microservice"

# Build the users-microservice image
docker build -t users-microservice .

# Run the users-microservice container
docker run -d --name users-microservice --network microservice-net -p 8080:8080 users-microservice
